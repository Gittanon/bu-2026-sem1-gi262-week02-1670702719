...
namespace Assignment
{
    public class Assignment : MonoBehaviour
    {
        ...
        public void AS01_RandomItemDrop()
        {
            ...
            GameObject selectedItem = as01_items[r];
        ...
        public void AS02_NestedLoopForCreate2DMap()
        {
            for (int y = 0; y < as02_rows; y++)
            {
                for (int x = 0; x < as02_columns; x++)
                {
                    ...
                    GameObject tilePrefab = as02_floorTiles[r];
        ...
        {
            for (int y = -1; y < as02_rows + 1; y++)
            {
                for (int x = -1; x < as02_columns + 1; x++)
                {
                    if (x == -1 || y == -1 || x == as02_rows || y == as02_columns)
                    {
                        int r = UnityEngine.Random.Range(0, as03_wall.Length);
                        GameObject tile = Instantiate(as03_wall[r], new Vector2(x, y), Quaternion.identity);
        ...
        public void AS11_SpawnEnemies()
        {
            ...
            {
                Instantiate(
                    as11_enemyPrefab,new Vector2(i + 1, 0),transform.rotation
                );
