        ...
        public void Start()
        {
            ...
            for (int y = 0; y < rows; y++)
            {
                for (int x = 0; x < columns; x++)
                {
                    ...
                    (floorTiles[r],new Vector2(x,y), Quaternion.identity);
            ...
            // 5. random foods
            int number = UnityEngine.Random.Range(2,3);
            for (int i = 0; i < number; i++)
            {
                int Foodxpositon = UnityEngine.Random.Range(0,columns);
                int Foodypositon = UnityEngine.Random.Range(0,rows);
            
                GameObject toInstantiate = foodTiles[UnityEngine.Random.Range(0, foodTiles.Length)];
                Instantiate(toInstantiate, new Vector2(Foodxpositon, Foodypositon), Quaternion.identity);
            }
